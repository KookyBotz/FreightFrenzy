    ...
    public void start() {
        ...
        CommandScheduler.getInstance().schedule(
                new SequentialCommandGroup(
                        ...
                        new DrivetrainCommand(new Pose(-15, 0, 90), robot, odometry, telemetry, 1000)
                                .alongWith(new PreloadRetractCommand(robot)),
                        new DrivetrainCommand(new Pose(-15, 34, 60), robot, odometry, telemetry, 1000)
                                .alongWith(new WaitCommand(1000)
                                        .andThen(new InstantCommand(() -> robot.intake.start()))),
                        new DrivetrainCommand(new Pose(-15, -10, 37.5), robot, odometry, telemetry, 1000)
                                .alongWith(new AllianceHubPreCommand(robot)
                                        .andThen(new AllianceHubAutoCommand(robot))),
                        new PreloadDumpCommand(robot),
                        new DrivetrainCommand(new Pose(-13, -5, 90), robot, odometry, telemetry, 1000)
                                .alongWith(new PreloadRetractCommand(robot)),
                        new DrivetrainCommand(new Pose(-13, 36, 60), robot, odometry, telemetry, 1000)
                                .alongWith(new WaitCommand(1000)
                                        .andThen(new InstantCommand(() -> robot.intake.start()))),
                        new DrivetrainCommand(new Pose(-13, -15, 37.5), robot, odometry, telemetry, 1000)
                                .alongWith(new AllianceHubPreCommand(robot)
                                        .andThen(new AllianceHubAutoCommand(robot))),
                        new PreloadDumpCommand(robot),
                        new DrivetrainCommand(new Pose(-13, -8, 90), robot, odometry, telemetry, 1000)
                                .alongWith(new PreloadRetractCommand(robot)),
                        new DrivetrainCommand(new Pose(-13, 30, 90), robot, odometry, telemetry, 1000)
                )
        );
    }
